...
import org.fluentcodes.projects.elasticobjects.calls.db.StatementFind;
...
public class StatementFindTest {
    @Before
    public void init() {
    ...
    public void anObject__readFirst__1() {
        EoRoot eo = ObjectProvider.createEo();
        StatementFind findStatement = new StatementFind("Select * from AnObject", eo);
        DbConfig dbConfig = (DbConfig) eo.getConfigMaps().find(HostConfig.class, DbConfig.H2_BASIC);
        Object result = findStatement.readFirst(dbConfig.getConnection(), eo.getConfigMaps());//, new ListParams().setRowHead(0).setRowStart(0));
    ...
    public void anObject_myString_notExist__execute__0() {
        EoRoot eo = ObjectProvider.createEo();
        StatementFind findStatement = new StatementFind("Select * from AnObject where myString='notExist'", eo);
        DbConfig dbConfig = (DbConfig) eo.getConfigMaps().find(HostConfig.class, DbConfig.H2_BASIC);
        int value = findStatement.execute(dbConfig.getConnection());
    ...
    public void anObject_myString_value1__execute__1() {
        EoRoot eo = ObjectProvider.createEo();
        StatementFind findStatement = new StatementFind("Select * from AnObject where myString='value1'", eo);
        DbConfig dbConfig = (DbConfig) eo.getConfigMaps().find(HostConfig.class, DbConfig.H2_BASIC);
        int value = findStatement.execute(dbConfig.getConnection());
        Assertions.assertThat(value).isEqualTo(1);
    }
